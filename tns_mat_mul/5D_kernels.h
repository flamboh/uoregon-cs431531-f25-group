    ...
    int uncontracted_mode, int shmem_size, int wavefront_size = 64) 
{
    ...
    if(active){
        ...
        int m3_bits = ceiling_log2(dims[2]);
        int m4_bits = ceiling_log2(dims[3]);
        int m5_bits = ceiling_log2(dims[4]); 
        ...
        m3_index = extract_mode_nd(lin_index, 3, masks, bit_widths, RANK_DIMS, block); 
        m4_index = extract_mode_nd(lin_index, 4, masks, bit_widths, RANK_DIMS, block);
        m5_index = extract_mode_nd(lin_index, 5, masks, bit_widths, RANK_DIMS, block); 
    }
    
    // --- 2. TTM Contraction Setup (Generalized Logic) ---
    
    // Arrays to hold the parameters of the 4 contracted factors
    int contracted_ranks[4]; 
    int input_indices[4];    
    T* factor_matrices[4]; 

    // Helper arrays for dynamic assignment
    int* all_indices[5] = {&m1_index, &m2_index, &m3_index, &m4_index, &m5_index};
    T* all_factor_mats[5] = {d_U1, d_U2, d_U3, d_U4, d_U5};
    const int all_ranks[5] = {R1, R2, R3, R4, R5};
    ...
    // Assign consolidated variables (Using C1-C4 for contracted ranks)
    const int C1 = contracted_ranks[0];
    const int C2 = contracted_ranks[1];
    const int C3 = contracted_ranks[2];
    const int C4 = contracted_ranks[3];
    ...
    for(int r1 = 0; r1 < C1; r1++){
        // Factor access stride is always its own rank (C1, C2, C3, C4 are Ranks)
        ...
        for(int r2 = 0; r2 < C2; r2++){
            ...
            for(int r3 = 0; r3 < C3; r3++){
                ...
                for(int r4 = 0; r4 < C4; r4++){
                    T u4_val = F4[I4 * C4 + r4 * active];

                    // Contraction is X * U1 * U2 * U3 * U4
                    T contrib = thread_val * u1_val * u2_val * u3_val * u4_val * active;
                    
                    // Col Index (Row-Major based on Ranks C1..C4)
                    int output_col_index = r1 * C2 * C3 * C4 + 
                                            r2 * C3 * C4 + 
                                            r3 * C4 + 
                                            r4;
